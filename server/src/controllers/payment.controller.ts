...
export class PaymentController {
  ...
  async getPayments(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      if (req.user?.roleSlug === 'collector') { conditions.push(`(l.collector_id = $${paramIndex} OR (l.collector_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM users WHERE id = l.collector_id AND is_active = true)))`); values.push(req.user.userId); paramIndex++; }
