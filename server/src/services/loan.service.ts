    ...
    const aggQuery = isCollector
      ? `
        SELECT
          ...
          (SELECT COUNT(*) FROM amortization_schedules a JOIN loans l ON l.id = a.loan_id WHERE a.due_date < CURRENT_DATE AND COALESCE(a.paid_amount,0) < a.total_due AND l.collector_id = $1) as overdue_count,
      ...
      : `
        SELECT
          ...
          (SELECT COUNT(*) FROM amortization_schedules WHERE due_date < CURRENT_DATE AND COALESCE(paid_amount,0) < total_due) as overdue_count,
