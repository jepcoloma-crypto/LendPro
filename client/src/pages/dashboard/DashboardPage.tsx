...
import { Panel, Loader, Row, Col, Tag } from 'rsuite';
...
export const DashboardPage = () => {
  ...
  const trendData = allMonths.map((month) => {
    ...
    const collections = payments?.collected || 0;
    const released = releases?.released || 0;
    return { month, collections, releases: released, net: released - collections, interest: payments?.interest || 0, penalty: payments?.penalty || 0 };
  });

  const trendMonths = trendData.map((d: any) => d.month);
